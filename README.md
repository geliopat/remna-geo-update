...
Источники списков (по умолчанию):
...
> 💡 Источники не зашиты намертво — можно подставить **любые** свои `geoip.dat` /
> `geosite.dat` (хоть [Loyalsoldier](https://github.com/Loyalsoldier/v2ray-rules-dat),
> хоть собственную сборку) через переменные `GEOIP_URL` / `GEOSITE_URL`.
...
- Раз в сутки **строго в 04:00** (`OnCalendar=*-*-* 04:00:00`, `Persistent=true` —
  пропущенный из-за выключенного сервера запуск догонится) сверяет свежий файл с
  текущим байт-в-байт.
...
TIMER_CALENDAR="*-*-* 04:00:00"               # время запуска (по умолчанию строго 04:00)
TIMER_DELAY=1800                              # случайный разброс, сек (0 = строго по времени)
...
GEOIP_URL=https://example.com/geoip.dat       # любой свой источник geoip.dat
GEOSITE_URL=https://example.com/geosite.dat   # любой свой источник geosite.dat
```
